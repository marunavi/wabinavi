...
(function(){
  ...
  var dynCache = {};
  /* 楽天トラベルAPI 近隣ホテル */   var RAKUTEN_APP_ID='b91a95fd-2188-46ff-a466-88040d44cf4a';   var RAKUTEN_ACCESS_KEY='pk_efKQ9Eyzu3LgUSWVTDSBHxLUCva8n76zdA6MLpt9Sof';   var RAKUTEN_AFFILIATE_ID='55c72f69.237de1c5.55c72f6a.3905e7a2';   var rakCache={};   function wcRakutenHotels(lat,lng,cb){     try{       if(lat==null||lng==null){cb(null);return;}       var url='https://openapi.rakuten.co.jp/engine/api/Travel/SimpleHotelSearch/20170426'         +'?applicationId='+encodeURIComponent(RAKUTEN_APP_ID)         +'&accessKey='+encodeURIComponent(RAKUTEN_ACCESS_KEY)         +'&affiliateId='+encodeURIComponent(RAKUTEN_AFFILIATE_ID)         +'&format=json&datum=1&hits=30&searchRadius=3&latitude='+lat+'&longitude='+lng;       var _cbn='__wabiRak'+(Math.random()*1e9|0);window[_cbn]=function(j){         var listH=(j&&j.hotels)||[],out=[];         for(var i=0;i<listH.length;i++){           var arr=listH[i]&&listH[i].hotel;if(!arr)continue;           var b=null;for(var k=0;k<arr.length;k++){if(arr[k]&&arr[k].hotelBasicInfo){b=arr[k].hotelBasicInfo;break;}}           if(!b)continue;           var rating=+b.reviewAverage||0,price=+b.hotelMinCharge||0;           if(rating<4.0)continue;           if(!(price>=7000&&price<=30000))continue;           var area=(b.address2||b.address1||'');           out.push({name:b.hotelName||'ホテル',             genre:(b.hotelSpecial?String(b.hotelSpecial).replace(/<[^>]*>/g,'').slice(0,16):'ホテル・旅館'),             rating:rating,reviews:+b.reviewCount||0,             walk:(area?String(area).slice(0,12):'巡拝の拠点に'),             price:'¥'+Number(price).toLocaleString('ja-JP')+'〜',             ai:Math.min(99,Math.round(rating*19+Math.min(+b.reviewCount||0,1000)/125)),             img:I('🏨',G.hotel),             photoUrl:b.hotelImageUrl||b.hotelThumbnailUrl||b.roomImageUrl||null,             url:b.hotelInformationUrl||b.planListUrl||null});         }         out.sort(function(a,b){return (b.rating-a.rating)||(b.reviews-a.reviews);});         cb(out.slice(0,6));       };var _s=document.createElement('script');_s.src=url+'&callback='+_cbn;_s.onerror=function(){cb(null);};document.head.appendChild(_s);     }catch(e){cb(null);}   }   function applyRakuten(route,base){     if(!base||!route||!route.spots||!route.spots[0])return base;     var s0=route.spots[0],rk=s0.name+'|'+s0.lat;     if(rakCache[rk]&&rakCache[rk].length){var c={};for(var k in base)c[k]=base[k];c.hotel=rakCache[rk];return c;}     return base;   }   function dynKey(route){ return route.spots.map(function(s){return s.name;}).join('|'); }
