...
(function(){
  ...
  // ─────────────────────────────────────────
  function wcShowAll(ck){var d=fetchNearby(state.route),items=(d[ck]||[]),T={gourmet:'グルメ',cafe:'カフェ・スイーツ',sight:'観光スポット',exp:'体験・アクティビティ',hotel:'宿泊施設'};var ov=document.getElementById('wcAllOv');if(!ov){ov=document.createElement('div');ov.id='wcAllOv';document.body.appendChild(ov);}ov.style.cssText='position:fixed;inset:0;z-index:280;background:#F8F5EF;overflow-y:auto;-webkit-overflow-scrolling:touch;';var h='<div style="position:sticky;top:0;background:#fff;display:flex;align-items:center;gap:8px;padding:14px 16px;border-bottom:1px solid #eee8dc;z-index:2"><span id="wcAllBack" style="font-size:24px;color:#a83320;cursor:pointer;padding:0 8px">‹</span><span style="font-family:Shippori Mincho,serif;font-weight:800;font-size:15px;color:#2a2018">'+(T[ck]||'')+'（'+items.length+'件）</span></div><div style="max-width:500px;margin:0 auto;padding:12px 16px 60px">';items.forEach(function(it,i){h+='<div class="wc-allc" data-i="'+i+'" style="display:flex;gap:12px;background:#fff;border:1px solid #e8dfcd;border-radius:16px;padding:10px;margin-bottom:10px;cursor:pointer"><div style="width:100px;height:76px;flex:0 0 100px;border-radius:12px;overflow:hidden;background:'+it.img.grad+';display:flex;align-items:center;justify-content:center;font-size:28px;color:#fff">'+(it.photoUrl?'<img src="'+esc(it.photoUrl)+'" style="width:100%;height:100%;object-fit:cover">':it.img.emoji)+'</div><div style="flex:1;min-width:0"><div style="font-family:Shippori Mincho,serif;font-weight:800;font-size:13px;color:#2a2018">'+it.name+'</div><div style="font-size:11px;color:#a89a80;margin-top:3px">★'+it.rating.toFixed(1)+'（'+it.reviews+'）・'+it.walk+'</div>'+(it.price?'<div style="font-size:12px;font-weight:800;color:#a83320;margin-top:3px">'+it.price+'</div>':'')+'</div></div>';});h+='</div>';ov.innerHTML=h;ov.style.display='block';document.getElementById('wcAllBack').onclick=function(){ov.style.display='none';};ov.querySelectorAll('.wc-allc').forEach(function(c){c.onclick=function(){var ix=+c.getAttribute('data-i');if(ck==='hotel'){var it2=(fetchNearby(state.route).hotel||[])[ix];if(it2&&it2.url){window.open(it2.url,'_blank');return;}}ov.style.display='none';openSpot(ck,ix);};});}
